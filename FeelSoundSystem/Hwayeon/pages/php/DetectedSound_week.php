...
<html>
  <head>
    <meta charset="utf-8">
    <title>DetectedSound_week</title>
  ...
  <body>
    <?php
      ...
      while ($baby_cnt < 6) {
        $baby_records[$baby_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_SUB(CURDATE(),INTERVAL '".$baby_day."' DAY) AND S_NUM = 0");
        $baby_day++;
        $baby_cnt++;
      }
      $baby_records[6] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%' AND S_NUM = 0;");
      $baby_cnt = 0;
      ...
      while ($car_cnt < 6) {
        $car_records[$car_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_SUB(CURDATE(),INTERVAL '".$car_day."' DAY) AND S_NUM = 1");
        $car_day++;
        $car_cnt++;
      }
      $car_records[6] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%' AND S_NUM = 1;");
      $car_cnt = 0;
      ...
      while ($dog_cnt < 6) {
        $dog_records[$dog_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_SUB(CURDATE(),INTERVAL '".$dog_day."' DAY) AND S_NUM = 2");
        $dog_day++;
        $dog_cnt++;
      }
      $dog_records[6] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%' AND S_NUM = 2;");
      $dog_cnt = 0;
      ...
      while ($jack_cnt < 6) {
        $jack_records[$jack_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_SUB(CURDATE(),INTERVAL '".$jack_day."' DAY) AND S_NUM = 3");
        $jack_day++;
        $jack_cnt++;
      }
      $jack_records[6] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%' AND S_NUM = 3;");
      $jack_cnt = 0;
      ...
      while ($siren_cnt < 6) {
        $siren_records[$siren_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_SUB(CURDATE(),INTERVAL '".$siren_day."' DAY) AND S_NUM = 4");
        $siren_day++;
        $siren_cnt++;
      }
      $siren_records[6] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%' AND S_NUM = 4;");
      $siren_cnt = 0;
      ...
      //원형그래프 데이터
      $total_records_sql = "SELECT COUNT(*) AS `total` FROM HW_RECORD WHERE DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
      $baby_records_sql = "SELECT COUNT(*) AS `baby_cry` FROM HW_RECORD WHERE S_NUM=0 AND DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
      $car_records_sql = "SELECT COUNT(*) AS `car_horn` FROM HW_RECORD WHERE S_NUM=1 AND DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
      $dog_records_sql = "SELECT COUNT(*) AS `dog_bark` FROM HW_RECORD WHERE S_NUM=2 AND DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
      $jack_records_sql = "SELECT COUNT(*) AS `jack_hammer` FROM HW_RECORD WHERE S_NUM=3 AND DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
      $siren_records_sql = "SELECT COUNT(*) AS `siren` FROM HW_RECORD WHERE S_NUM=4 AND DATE(TIME) >= DATE_SUB(CURDATE(),INTERVAL 7 DAY)";
    ...
    <div class="box">
      <!-- datepicker -->
      <form action="SearchWeek.php" method="post">
        ...
        <h1 class="title_h1"> 최근 7일간 데이터 </h1>
    ...
    <script>
      ...
      var chartData = {
              labels: ["2020/10/05", "2020/10/06", "2020/10/07", "2020/10/08", "2020/10/09", "2020/10/10", "<? echo $cur_YMD; ?>"],
              datasets: [{ data: [ <? echo $n_baby[5]; ?>, <? echo $n_baby[4]; ?>, <? echo $n_baby[3]; ?>, <? echo $n_baby[2]; ?>, <? echo $n_baby[1]; ?>, <? echo $n_baby[0]; ?>,
                <? echo $n_baby[6]; ?>],
            ...
            {
              data: [<? echo $n_car[5]; ?>, <? echo $n_car[4]; ?>, <? echo $n_car[3]; ?>, <? echo $n_car[2]; ?>, <? echo $n_car[1]; ?>, <? echo $n_car[0]; ?>,
                <? echo $n_car[6]; ?>],
              backgroundColor: 'transparent',
            ...
            {
              data: [<? echo $n_dog[5]; ?>, <? echo $n_dog[4]; ?>, <? echo $n_dog[3]; ?>, <? echo $n_dog[2]; ?>, <? echo $n_dog[1]; ?>, <? echo $n_dog[0]; ?>,
                <? echo $n_dog[6]; ?>],
              backgroundColor: 'transparent',
            ...
            {
              data: [<? echo $n_jack[5]; ?>, <? echo $n_jack[4]; ?>, <? echo $n_jack[3]; ?>, <? echo $n_jack[2]; ?>, <? echo $n_jack[1]; ?>, <? echo $n_jack[0]; ?>
              , <? echo $n_jack[6]; ?>],
              backgroundColor: 'transparent',
            ...
            {
              data: [<? echo $n_siren[5]; ?>, <? echo $n_siren[4]; ?>, <? echo $n_siren[3]; ?>, <? echo $n_siren[2]; ?>, <? echo $n_siren[1]; ?>, <? echo $n_siren[0]; ?>,
              <? echo $n_siren[6]; ?>],
              backgroundColor: 'transparent',
