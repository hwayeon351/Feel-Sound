...
<html>
  ...
  <body>
    <?php
      ...
      while ($baby_cnt < 6) {
        $baby_records[$baby_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_ADD('".$datepicker_to."', INTERVAL -'".$baby_day."' DAY) AND S_NUM = 0");
        $baby_day++;
        $baby_cnt++;
      }
      ...
      while ($car_cnt < 6) {
        $car_records[$car_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_ADD('".$datepicker_to."', INTERVAL -'".$car_day."' DAY) AND S_NUM = 1");
        $car_day++;
        $car_cnt++;
      }
      ...
      while ($dog_cnt < 6) {
        $dog_records[$dog_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_ADD('".$datepicker_to."', INTERVAL -'".$dog_day."' DAY) AND S_NUM = 2");
        $dog_day++;
        $dog_cnt++;
      }
      ...
      while ($jack_cnt < 6) {
        $jack_records[$jack_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_ADD('".$datepicker_to."', INTERVAL -'".$jack_day."' DAY) AND S_NUM = 3");
        $jack_day++;
        $jack_cnt++;
      }
      ...
      while ($siren_cnt < 6) {
        $siren_records[$siren_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE DATE(TIME) = DATE_ADD('".$datepicker_to."', INTERVAL -'".$siren_day."' DAY) AND S_NUM = 4");
        $siren_day++;
        $siren_cnt++;
      }
      ...
      //원형그래프 데이터
      $total_records_sql = "SELECT COUNT(*) AS `total` FROM HW_RECORD WHERE DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
      $baby_records_sql = "SELECT COUNT(*) AS `baby_cry` FROM HW_RECORD WHERE S_NUM=0 AND DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
      $car_records_sql = "SELECT COUNT(*) AS `car_horn` FROM HW_RECORD WHERE S_NUM=1 AND DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
      $dog_records_sql = "SELECT COUNT(*) AS `dog_bark` FROM HW_RECORD WHERE S_NUM=2 AND DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
      $jack_records_sql = "SELECT COUNT(*) AS `jack_hammer` FROM HW_RECORD WHERE S_NUM=3 AND DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
      $siren_records_sql = "SELECT COUNT(*) AS `siren` FROM HW_RECORD WHERE S_NUM=4 AND DATE(TIME) BETWEEN '".$datepicker_from."' AND '".$datepicker_to."'";
    ...
    <div class="box">
      ...
      <form action="SearchWeek.php" method="post">
        ...
        <h1 class="title_h1"> <? echo $datepicker_from;?> ~ <? echo $datepicker_to;?> </h1>
    ...
    <script>
      ...
      var chartData = {
              labels: ["<? echo $datepicker_from; ?>", "2020/10/02", "2020/10/03", "2020/10/04", "2020/10/05", "2020/10/06", "<? echo $datepicker_to; ?>"],
