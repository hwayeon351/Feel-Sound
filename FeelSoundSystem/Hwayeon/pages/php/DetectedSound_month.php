...
<html>
  ...
  <body>
    <?php
      ...
      $baby_records = array();
      $baby_day = array();
      $day = ["$cur_YM/01","$cur_YM/05","$cur_YM/10","$cur_YM/15","$cur_YM/20","$cur_YM/25", "$cur_YM/30"];
      $baby_cnt = 0;
      echo "$baby_day[0]";

      while ($baby_cnt < 7) {
        $baby_records[$baby_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$day[$baby_cnt]."%' AND S_NUM = 0");
        $baby_day++;
        $baby_cnt++;
      }
      $baby_cnt = 0;
      ...
      while ($car_cnt < 7) {
        $car_records[$car_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$day[$car_cnt]."%' AND S_NUM = 1");
        $car_day++;
        $car_cnt++;
      }
      $car_cnt = 0;
      ...
      while ($dog_cnt < 7) {
        $dog_records[$dog_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$day[$dog_cnt]."%' AND S_NUM = 2");
        $dog_day++;
        $dog_cnt++;
      }
      $dog_cnt = 0;
      ...
      while ($jack_cnt < 7) {
        $jack_records[$jack_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$day[$jack_cnt]."%' AND S_NUM = 3");
        $jack_day++;
        $jack_cnt++;
      }
      $jack_cnt = 0;
      ...
      while ($siren_cnt < 7) {
        $siren_records[$siren_cnt] = mysqli_query($con,"SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$day[$siren_cnt]."%' AND S_NUM = 4");
        $siren_day++;
        $siren_cnt++;
      }
      $siren_cnt = 0;
      ...
      //원형그래프 데이터
      $total_records_sql = "SELECT COUNT(*) AS `total` FROM HW_RECORD WHERE TIME LIKE '".$cur_YM."%'";
      $baby_records_sql = "SELECT COUNT(*) AS `baby_cry` FROM HW_RECORD WHERE S_NUM=0 AND TIME LIKE '".$cur_YM."%'";
      $car_records_sql = "SELECT COUNT(*) AS `car_horn` FROM HW_RECORD WHERE S_NUM=1 AND TIME LIKE '".$cur_YM."%'";
      $dog_records_sql = "SELECT COUNT(*) AS `dog_bark` FROM HW_RECORD WHERE S_NUM=2 AND TIME LIKE '".$cur_YM."%'";
      $jack_records_sql = "SELECT COUNT(*) AS `jack_hammer` FROM HW_RECORD WHERE S_NUM=3 AND TIME LIKE '".$cur_YM."%'";
      $siren_records_sql = "SELECT COUNT(*) AS `siren` FROM HW_RECORD WHERE S_NUM=4 AND TIME LIKE '".$cur_YM."%'";
    ...
    <div class="box">
      ...
      <form action="SearchMonth.php" method="post">
        ...
        <h1 class="title_h1"> <? echo $cur_Y; ?>년 <? echo $cur_m; ?>월 데이터 </h1>
    ...
    <script>
      ...
      var chartData = {
              labels: ["<? echo $day[0]; ?>", "<? echo $day[1]; ?>", "<? echo $day[2]; ?>", "<? echo $day[3]; ?>", "<? echo $day[4]; ?>", "<? echo $day[5]; ?>"
              ,"<? echo $day[6]; ?>"],
