...
<html>
  ...
  <body>
    <?php
      ...
      $total_records_sql = "SELECT COUNT(*) FROM HW_RECORD WHERE TIME LIKE '".$cur_YMD."%'";
      ...
      if(!($total_sql_result->num_rows)){
        $num_total = 0;
      }
      else{
        while($total_result = mysqli_fetch_assoc($total_sql_result))
        {
          $num_total = (int)$total_result['COUNT(*)'];
        }
      }
      /*
      if(!$total_sql_result){
...
*/
    ...
    <div class="box">
      ...
      <h1 class="title_h1"> <?echo $fulldate ?></h1>
      <h3 class="graph_title">
          < 누적 소리 인식 횟수 >
        </h3>
        <div class="total">
          Total: <? echo $num_total; ?> <br>
        </div>
